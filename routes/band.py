...
async def get_bands(genre: GenreUrl | None = None, q: Annotated[str | None, Query(max_length=10)] = None, session: Session = Depends(get_session)) -> List[Band]:
...
async def get_band(band_id: Annotated[int, Path(title="band_id")], session: Session = Depends(get_session)) -> Band:
